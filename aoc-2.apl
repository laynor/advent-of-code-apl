...
∇R←C SPLIT∆STRING STR;NC
  NC← S≠C
  R← NC \ NC / S
∇

∇R←READ∆MEASURE S;NOTX
  R← ⍎(S≠'x') \ (S≠'x') / S
∇

∇R←PREPROCESS S;NX;NNL;NOTCRAP
  NX ← (S≠'x')
  NNL ← (S≠(⎕UCS 10))
  NOTCRAP ← NX ∧ NNL
  R←1000 3⍴⍎NOTCRAP \ NOTCRAP / S
∇

∇R←PAPER M
  ROWS ← (⍴M)[1]
  ⍝ UGLY!
  SIDES ← (ROWS 1⍴(M[;1]×M[;2])),(ROWS 1⍴(M[;1]×M[;3])),(ROWS 1⍴(M[;2]×M[;3]))
  R← (+/(2×SIDES)) + ⌊/SIDES
∇

∇R←Day2∆1
  I← FIO∆read_file '/home/ale/src/apl/advent-of-code-apl/input2.txt'
  R← +/PAPER PREPROCESS I
∇
...
∇R←Day2∆2
  I← FIO∆read_file '/home/ale/src/apl/advent-of-code-apl/input2.txt'
  MM ← ,/PREPROCESS I ⍝ Make each row a nested vector
  R← +/RIBBON¨MM
∇
